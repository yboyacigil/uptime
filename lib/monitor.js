...
Monitor.prototype.pollChecksNeedingPoll = function(callback) {
  ...
  this.findGlobalMute(function (err, mute) {
    if (err) {
      console.log(err)
    ...
    var muted = function(start, last) {
      var m = moment(start);
      var diff = m.diff(Date.now(), 'minutes');
      return (diff + last) > 0  
    }  

    if (!muted(mute.start, mute.last)) {
      self.findChecksNeedingPoll(function(err, checks) {
        if (err) {
          console.error(err);
          if (callback) callback(err);
          return;
        ...
        checks.forEach(function(check) {
          // if check is in maintenance, skip polling
          if (check.pollerParams && check.pollerParams.muteStart && check.pollerParams.muteStart) {
            if (muted(check.pollerParams.muteStart, check.pollerParams.muteLast)) {
              console.log('Check (' + check.name + ') is muted starting ' + check.pollerParams.muteStart + ' for ' + check.pollerParams.muteLast + ' mins');
              return;
            }
          }
      ...
      console.log('Global MUTE is active starting: ' + mute.start + ' for ' + mute.last + ' mins');
    }
  });
};

Monitor.prototype.findGlobalMute = function(callback) {
  var options = url.parse(this.config.apiUrl + '/mute/global');
  this.applyApiHttpOptions(options);
  var self = this;
  http.get(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(self.config.apiUrl + '/mute/global resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(self.config.apiUrl + '/mute/global resource not available: ' + e.message));
  });
};
